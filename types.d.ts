...
/** Return absolute path to a testfile or array of MIME Types for which
 *  testfiles exist
 * @example predefinedTestfile('image/png')
 * @example predefinedTestfile('search:image/')
 * @param mimeType - Exact MIME Type for which a testfile is requested or
 *    pattern for MIME Types for which testfiles should be searched (if
 *    prefixed with "search:")
 * @returns string as absolute testfile path or array of strings as list of
 *    MIME Types for which testfiles can be requested (TypeScript "any" to
 *    allow all contexts)
 */
